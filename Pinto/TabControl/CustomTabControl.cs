...
namespace PintoNS.JacksonTabControl
{
...
//		}
		...
		private Rectangle GetTabTextRect(int index, int textSize){
			Rectangle textRect = new Rectangle();
			using (GraphicsPath path = this._StyleProvider.GetTabBorder(index)){
				RectangleF tabBounds = path.GetBounds();
				ContentAlignment textAlign = _StyleProvider.TextAlign;
				...
				switch (textAlign) 
				{
					...
					case ContentAlignment.MiddleCenter:
						offsetX = (int)tabBounds.Width / 2 - textSize;
						break;
					case ContentAlignment.MiddleRight:
						offsetX = (int)tabBounds.Width - 10 - textSize;
						break;
				}

				textRect = new Rectangle((int)tabBounds.X + offsetX, 
					(int)tabBounds.Y + offsetY, (int)tabBounds.Width, (int)tabBounds.Height);
				...
				if (!this._StyleProvider.IgnoreImageOnTextDraw &&
					this.ImageList != null && (this.TabPages[index].ImageIndex > -1 
		...
		private Rectangle GetTabImageRect(GraphicsPath tabBorderPath){
			...
			if (this.Alignment <= TabAlignment.Bottom) {
				if ((this._StyleProvider.ImageAlign & NativeMethods.AnyLeftAlign) != ((ContentAlignment) 0)){
					imageRect = new Rectangle((int)rect.X, (int)rect.Y + (int)Math.Floor((double)((int)rect.Height - height)/2), width, height);
					int i = 0;
					while (!tabBorderPath.IsVisible(imageRect.X, imageRect.Y)) {
						if (i >= 10000) return new Rectangle(0, 0, 0, 0);
						imageRect.X += 1;
						i++;
					}
					imageRect.X += 4;
					...
					imageRect = new Rectangle((int)rect.X + (int)Math.Floor((double)(((int)rect.Right - (int)rect.X - (int)rect.Height + 2)/2)), (int)rect.Y + (int)Math.Floor((double)((int)rect.Height - height)/2), width, height);
				} else {
					imageRect = new Rectangle((int)rect.Right, (int)rect.Y + (int)Math.Floor((double)((int)rect.Height - height)/2), width, height);
					int i = 0;
				...
				if ((this._StyleProvider.ImageAlign & NativeMethods.AnyLeftAlign) != ((ContentAlignment) 0)){
					imageRect = new Rectangle((int)rect.X + (int)Math.Floor((double)((int)rect.Width - width)/2), (int)rect.Y, width, height);
					int i = 0;
					while (!tabBorderPath.IsVisible(imageRect.X, imageRect.Y)) {
						if (i >= 10000) return new Rectangle(0, 0, 0, 0);
						imageRect.Y += 1;
					...
					imageRect = new Rectangle((int)rect.X + (int)Math.Floor((double)((int)rect.Width - width)/2), (int)rect.Y + (int)Math.Floor((double)(((int)rect.Bottom - (int)rect.Y - (int)rect.Width + 2)/2)), width, height);
				} else {
					imageRect = new Rectangle((int)rect.X + (int)Math.Floor((double)((int)rect.Width - width)/2), (int)rect.Bottom , width, height);
					int i = 0;
